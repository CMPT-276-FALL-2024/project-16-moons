    ...
    const fetchSpecificRecipe = async () => {
      try {
        const response = await fetch(
          `https://api.edamam.com/api/recipes/v2?app_id=${appId}&app_key=${apiKey}&type=public&q=${recipeToFetch}`
        );
        ...
        <div className="instruct">
          ...
          {data == null && (
            <p className="subtitle">
              Enter the name of the recipe you're looking for in the search bar
              below.
            </p>
          )}
        ...
        {data && data.hits.length > 0 && (
          <div className="recipe-cards-container">
            ...
            {data.hits.slice(0, 50).map((hit, index) => (
              <RecipeCard key={index} data={hit} searchInput={recipeToFetch} />
            ))}
          </div>
        )}
      </section>
