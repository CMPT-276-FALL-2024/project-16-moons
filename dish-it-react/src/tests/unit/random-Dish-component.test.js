...
describe("GenerateRandomDish", () => {
  test("Correct API -> Load the dish details", async () => {
    ...
    const randomFoodlement = await screen.findByText(/Foodista/);
    // Check for certain tags is in the dom!
    expect(randomFoodlement).toBeInTheDocument();
  });
